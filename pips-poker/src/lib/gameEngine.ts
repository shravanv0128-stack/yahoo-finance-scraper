...
export async function startNewHand(supabase: SupabaseClient, roomId: string) {
  ...
  for (const player of players) {
    const ante = Math.min(ANTE_AMOUNT, player.chip_stack);
    pot += ante;
...
export async function applyBettingAction(
  ...
  if (roundComplete) {
    ...
    nextActiveSeat = activePlayers.length > 0 ? activePlayers[0].seat : null;
  } else {
  ...
  await supabase
    ...
    .update({
      ...
      min_raise: roundComplete ? 1 : result.state.minRaise,
 ...
 */
export async function runShowdown(supabase: SupabaseClient, roomId: string, handId: string) {
  const { data: gameState, error: gsError } = await supabase
    .from("game_state")
    .select("*")
    .eq("room_id", roomId)
  ...
  const community: Card[] = gameState.community_cards;
  ...
  for (const cp of contenders) {
    ...
    const bestHand =
      contenders.length === 1 ? null : evaluateBestHand(holeCards, community);
  ...
  if (contenders.length === 1) {
    ...
    // Split the pot (accounting for side pots from all-ins) 50/50 between
    // the poker half and the pip half. Each side pot is split into a poker
    // sub-pot and pip sub-pot, both restricted to that side pot's eligible
    // players, mirroring how side pots normally distribute betting chips.
    for (const sidePot of sidePots) {
      const eligibleEntries = entries.filter((e) => sidePot.eligibleHandPlayerIds.includes(e.hand_player_id));
      if (eligibleEntries.length === 0) continue;

      const pokerHalf = sidePot.amount / 2;
      const pipHalf = sidePot.amount - pokerHalf; // ensures the two halves sum exactly to sidePot.amount
  ...
  // Persist: copy revealed cards, update chip stacks (hand_players + players table).
  ...
  await supabase
    .from("game_state")
    .update({ phase: "hand_complete" as GamePhase, pot: 0, active_seat: null, updated_at: new Date().toISOString() })
    .eq("room_id", roomId);
  await supabase
    .from("hands")
